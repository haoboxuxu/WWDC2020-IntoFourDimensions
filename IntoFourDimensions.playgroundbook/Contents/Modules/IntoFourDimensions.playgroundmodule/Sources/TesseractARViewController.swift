...
public class TesseractARViewController: UIViewController, ARSessionDelegate, ARSCNViewDelegate {

    var arhcScene = SCNScene(named: "ar_hypercube.scn")
    ...
    public var sceneView = ARSCNView(frame: UIScreen.main.bounds)

    override public func viewDidLoad() {
        super.viewDidLoad()
        sceneView.delegate = self
        sceneView.session.delegate = self
        sceneView.scene = arhcScene!
    ...
    public func initNodes() {
        for i in 0..<16 {
            guard let node = arhcScene!.rootNode.childNode(withName: "sphere\(i)", recursively: true) else {return}
            sphereNodes.append(node)
        }
        
        for i in 0..<32 {
            guard let node = arhcScene!.rootNode.childNode(withName: "cylinder\(i)", recursively: true) else {return}
